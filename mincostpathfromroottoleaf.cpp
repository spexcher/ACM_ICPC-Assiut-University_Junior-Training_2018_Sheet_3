...
void solve()
{
  ...
  while (!q.empty())
  {
    ...
    if (g[curr.ff].size() == 1)
      leaves.pb(curr.ff);
    for (auto neighbour : g[curr.ff])
    {
      if (neighbour != curr.ss)
      {
        parent[neighbour] = curr.ff;
        dist[neighbour] = dist[curr.ff] + 1;
        q.push(mp(neighbour, curr.ff));
      }
    }
  }
  // print(parent);
  int minleaf = 2; // anything not equal to 1
  int currdistance = INT_MAX;
  for (auto x : leaves)
  {
    if (dist[x] < currdistance)
    {
      minleaf = x;
      currdistance = dist[x];
    }
